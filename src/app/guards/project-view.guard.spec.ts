...
describe('ProjectViewGuard', () => {
  ...
  const mockHttp = {
    get: () => {return (new Observable<any>((subscriber) => {
      subscriber.next({});
      }));
    }
  };
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      ...
      providers: [ProjectViewGuard, ProjectService, { provide: HttpClient, useValue: mockHttp}]
    });
  ...
  it('should return false when CurrentProject$.value returns null', inject([ProjectViewGuard], (guard: ProjectViewGuard) => {
    projectService.CurrentProject$.next(null);
    expect(component.canActivate()).toEqual(false);
    expect(routerSpy).toHaveBeenCalledWith([""]);
  }));

  it('should return true when CurrentProject$.value returns a project', inject([ProjectViewGuard], (guard: ProjectViewGuard) => {
